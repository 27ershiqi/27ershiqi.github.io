import{D as p}from"./index-6768f793.js";const g=`---\r
...
`,l=p([]);function w(e){const r=/^---\s*\n([\s\S]*?)\n---\s*\n([\s\S]*)$/,t=e.match(r);if(!t)return{frontmatter:{},content:e};const o=t[1],u=t[2],c={};return o.split(`
`).forEach(a=>{const s=a.indexOf(":");if(s>0){const m=a.substring(0,s).trim();let n=a.substring(s+1).trim();(n.startsWith('"')&&n.endsWith('"')||n.startsWith("'")&&n.endsWith("'"))&&(n=n.slice(1,-1)),n.startsWith("[")&&n.endsWith("]")&&(n=n.slice(1,-1).split(",").map(d=>d.trim().replace(/['"]/g,""))),c[m]=n}}),{frontmatter:c,content:u}}const C={"2025-11-04-hello-world.md":g,"2024-01-15-vue3-learning.md":v,"2024-02-01-markdown-editor.md":f,"2025-11-04-vue3-computed.md":h};async function i(){if(l.value.length>0)return l.value;const e=[];for(const[r,t]of Object.entries(C))try{const{frontmatter:o,content:u}=w(t),c=r.replace(".md","");e.push({slug:c,filename:r,title:o.title||"无标题",date:o.date||"未知日期",tags:o.tags||[],summary:o.summary||V(u),content:u,frontmatter:o})}catch(o){console.warn(`无法加载文章: ${r}`,o)}return e.sort((r,t)=>new Date(t.date)-new Date(r.date)),l.value=e,e}async function k(e){return(await i()).find(r=>r.slug===e)}function V(e,r=150){const t=e.replace(/#{1,6}\s+/g,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/\*(.*?)\*/g,"$1").replace(/`(.*?)`/g,"$1").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/```[\s\S]*?```/g,"").replace(/>\s+/g,"").replace(/[-*+]\s+/g,"").replace(/\n+/g," ").trim();return t.length>r?t.substring(0,r)+"...":t}async function y(){const e=await i(),r=new Set;return e.forEach(t=>{t.tags.forEach(o=>r.add(o))}),Array.from(r).sort()}const j=(e,r)=>{const t=e.__vccOpts||e;for(const[o,u]of r)t[o]=u;return t};export{k as V,i,j as x,y};
