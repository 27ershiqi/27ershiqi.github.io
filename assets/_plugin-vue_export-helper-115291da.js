import{r as p}from"./index-41e6f3df.js";const g=`---
...
`).forEach(u=>{const s=u.indexOf(":");if(s>0){const m=u.substring(0,s).trim();let r=u.substring(s+1).trim();(r.startsWith('"')&&r.endsWith('"')||r.startsWith("'")&&r.endsWith("'"))&&(r=r.slice(1,-1)),r.startsWith("[")&&r.endsWith("]")&&(r=r.slice(1,-1).split(",").map(d=>d.trim().replace(/['"]/g,""))),c[m]=r}}),{frontmatter:c,content:a}}const C={"2025-11-04-hello-world.md":g,"2024-01-15-vue3-learning.md":v,"2024-02-01-markdown-editor.md":f,"2025-11-04-vue3-computed.md":h};async function i(){if(l.value.length>0)return l.value;const t=[];for(const[e,n]of Object.entries(C))try{const{frontmatter:o,content:a}=w(n),c=e.replace(".md","");t.push({slug:c,filename:e,title:o.title||"无标题",date:o.date||"未知日期",tags:o.tags||[],summary:o.summary||k(a),content:a,frontmatter:o})}catch(o){console.warn(`无法加载文章: ${e}`,o)}return t.sort((e,n)=>new Date(n.date)-new Date(e.date)),l.value=t,t}async function V(t){return(await i()).find(n=>n.slug===t)}function k(t,e=150){const n=t.replace(/#{1,6}\s+/g,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/\*(.*?)\*/g,"$1").replace(/`(.*?)`/g,"$1").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/```[\s\S]*?```/g,"").replace(/>\s+/g,"").replace(/[-*+]\s+/g,"").replace(/\n+/g," ").trim();return n.length>e?n.substring(0,e)+"...":n}async function y(){const t=await i(),e=new Set;return t.forEach(n=>{n.tags.forEach(o=>e.add(o))}),Array.from(e).sort()}const x=(t,e)=>{const n=t.__vccOpts||t;for(const[o,a]of e)n[o]=a;return n};export{x as _,y as a,V as b,i as g};
