import{D as p}from"./index-2e85de24.js";const g=`---
...
`,l=p([]);function w(t){const e=/^---\s*\n([\s\S]*?)\n---\s*\n([\s\S]*)$/,o=t.match(e);if(!o)return{frontmatter:{},content:t};const n=o[1],u=o[2],c={};return n.split(`
`).forEach(a=>{const s=a.indexOf(":");if(s>0){const m=a.substring(0,s).trim();let r=a.substring(s+1).trim();(r.startsWith('"')&&r.endsWith('"')||r.startsWith("'")&&r.endsWith("'"))&&(r=r.slice(1,-1)),r.startsWith("[")&&r.endsWith("]")&&(r=r.slice(1,-1).split(",").map(d=>d.trim().replace(/['"]/g,""))),c[m]=r}}),{frontmatter:c,content:u}}const C={"2025-11-04-hello-world.md":g,"2024-01-15-vue3-learning.md":v,"2024-02-01-markdown-editor.md":f,"2025-11-04-vue3-computed.md":h};async function i(){if(l.value.length>0)return l.value;const t=[];for(const[e,o]of Object.entries(C))try{const{frontmatter:n,content:u}=w(o),c=e.replace(".md","");t.push({slug:c,filename:e,title:n.title||"无标题",date:n.date||"未知日期",tags:n.tags||[],summary:n.summary||V(u),content:u,frontmatter:n})}catch(n){console.warn(`无法加载文章: ${e}`,n)}return t.sort((e,o)=>new Date(o.date)-new Date(e.date)),l.value=t,t}async function k(t){return(await i()).find(e=>e.slug===t)}function V(t,e=150){const o=t.replace(/#{1,6}\s+/g,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/\*(.*?)\*/g,"$1").replace(/`(.*?)`/g,"$1").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/```[\s\S]*?```/g,"").replace(/>\s+/g,"").replace(/[-*+]\s+/g,"").replace(/\n+/g," ").trim();return o.length>e?o.substring(0,e)+"...":o}async function y(){const t=await i(),e=new Set;return t.forEach(o=>{o.tags.forEach(n=>e.add(n))}),Array.from(e).sort()}const j=(t,e)=>{const o=t.__vccOpts||t;for(const[n,u]of e)o[n]=u;return o};export{k as V,i,j as x,y};
