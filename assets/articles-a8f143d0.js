import{h as p}from"./vendor-c12888aa.js";const g=`---\r
...
`).forEach(u=>{const s=u.indexOf(":");if(s>0){const m=u.substring(0,s).trim();let o=u.substring(s+1).trim();(o.startsWith('"')&&o.endsWith('"')||o.startsWith("'")&&o.endsWith("'"))&&(o=o.slice(1,-1)),o.startsWith("[")&&o.endsWith("]")&&(o=o.slice(1,-1).split(",").map(d=>d.trim().replace(/['"]/g,""))),c[m]=o}}),{frontmatter:c,content:a}}const C={"2024-01-01-hello-world.md":g,"2024-01-15-vue3-learning.md":v,"2024-02-01-markdown-editor.md":f,"2025-11-04-vue3-computed.md":h};async function i(){if(l.value.length>0)return l.value;const e=[];for(const[r,n]of Object.entries(C))try{const{frontmatter:t,content:a}=w(n),c=r.replace(".md","");e.push({slug:c,filename:r,title:t.title||"无标题",date:t.date||"未知日期",tags:t.tags||[],summary:t.summary||k(a),content:a,frontmatter:t})}catch(t){console.warn(`无法加载文章: ${r}`,t)}return e.sort((r,n)=>new Date(n.date)-new Date(r.date)),l.value=e,e}async function V(e){return(await i()).find(n=>n.slug===e)}function k(e,r=150){const n=e.replace(/#{1,6}\s+/g,"").replace(/\*\*(.*?)\*\*/g,"$1").replace(/\*(.*?)\*/g,"$1").replace(/`(.*?)`/g,"$1").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/```[\s\S]*?```/g,"").replace(/>\s+/g,"").replace(/[-*+]\s+/g,"").replace(/\n+/g," ").trim();return n.length>r?n.substring(0,r)+"...":n}async function y(){const e=await i(),r=new Set;return e.forEach(n=>{n.tags.forEach(t=>r.add(t))}),Array.from(r).sort()}export{y as a,V as b,i as g};
